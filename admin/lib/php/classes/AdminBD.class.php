...
class AdminBD extends Admin {
    ...
    public function isAdmin($login, $password){
        try{
            $query = "select verifier_connexion(:nom_admin,:psw) as retour";
            $res = $this->_db->prepare($query);
            $res->bindValue(':nom_admin',$login);
            $res->bindValue(':psw',$password);
            $res->execute();
    ...
    public function getAdmin($login,$password){
        try{
            $query = "select * from admin where nom_admin=:login and psw = :password";
            $res = $this->_db->prepare($query);
            $res->bindValue(':nom_admin',$login);
            $res->bindValue(':psw',$password);
            $res->execute();
